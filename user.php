<?php
	...
	class User
	{
		...
		// =======================================================================
			...
			const DATABASE_LOCATION = "tmpdir/";
...
EOT;
		...
		function reset_password()
		{
			...
			$query = "SELECT username, token FROM graders WHERE email = '$email'";
			$username = "";
			$token = "";
			...
			$update = "UPDATE graders SET password = '$dbpassword' WHERE email= '$email'";
	...
	// ------------------
		...
		function register_user($username, $email, $sha1, &$registration_callback = false)
		{
			...
			if(User::unsafe_reporting) {
				$query = "SELECT username FROM graders WHERE username LIKE '$username'";
				foreach($this->database->query($query) as $data) {
					...
					return false; }}
			else{	$query = "SELECT username FROM graders";
				$usernames = array();
			...
			$query = "SELECT * FROM graders WHERE email = '$email'";
			foreach($this->database->query($query) as $data) {
				$this->info("user account for $username not created.");
			...
			$insert = "INSERT INTO graders (lastname, firstname, username, email, password, token, role, active, last) ";
			$insert .= "VALUES ('$lastname', '$firstname', '$username', '$email', '$dbpassword', '', 'user', 'true', '" . time() . "') ";
			$this->database->exec($insert);
			$query = "SELECT * FROM graders WHERE username = '$username'";
			foreach($this->database->query($query) as $data) {
		...
		{
			$update = "UPDATE graders SET active = 'false' WHERE username = '$username'";
			$this->database->exec($update);
			$this->resetSession();
		...
		{
			$delete = "DELETE FROM graders WHERE username = '$username'";
		...
		{
			if($username && $username !="" && $username !=User::GUEST_USER) {
				$query = "SELECT email FROM graders WHERE username = '$username'";
		...
		{
			if($username && $username !="" && $username !=User::GUEST_USER) {
				$query = "SELECT role FROM graders WHERE username = '$username'";
		...
		{
			$query = "SELECT token FROM graders WHERE username = '$username'";
		...
		function update_user_token($username, $sha1)
		{
			...
			$update = "UPDATE graders SET token = '$token' WHERE username = '$username'";
			$this->database->exec($update);

			// update the user's password
			$newpassword = $this->token_hash_password($username, $sha1, $token);
			$update = "UPDATE graders SET password = '$newpassword' WHERE username = '$username'";
			$this->database->exec($update);
		...
		{
			$update = "UPDATE graders SET active = 'true', last = '" . time() . "' WHERE username = '$username'";
			$this->database->exec($update);
		...
		function is_user_active($username)
		{
			$last = 0;
			$active = "false";
			$query = "SELECT last, active FROM graders WHERE username = '$username'";
			foreach($this->database->query($query) as $data) {
